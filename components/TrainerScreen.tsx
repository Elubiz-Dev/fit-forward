...
export default function TrainerScreen() {
  ...
  const {
    trainerMessages: messages, isTyping, msgCount, trainerSessions: sessions,
    currentTrainerSessionId: sessionId,
  ...
  const loadSessions = async () => {
    ...
    if (data) setSessions(data, 'trainer');
  };
    ...
    async function loadHistory() {
      ...
      if (sessionId) {
        ...
        if (sessions.length > 0) {
          setCurrentSessionId(sessions[0].id, 'trainer');
          return;
        }
  ...
  const handleSend = useCallback(async (overrideText?: string) => {
    ...
    if (!activeSessionId) {
      ...
      if (newSession) {
        ...
        setCurrentSessionId(activeSessionId, 'trainer');
        loadSessions();
      }
    }
